...
print("Root hash: ", encode(state.root_hash, "hex"))
print("Root node: ", state.root_node)
# they value at the key gets updated
...
state = trie.Trie(state.db, decode(primaryRoot, "hex"))
print(
    "- Now, let’s add an entry that uses the same key but with a different final nibble"
)
...
print("HP encoded key, in hex:", encode(common_prefix_key, "hex"))
